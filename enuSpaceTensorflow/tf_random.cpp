...
void* Create_Multinomial(std::string id, Json::Value pInputItem) {
	...
	{
		*pMultinomial = Multinomial(*pScope, *logit, *num_samples, attrs);
...
void* Create_ParameterizedTruncatedNormal(std::string id, Json::Value pInputItem) {
	...
	if (pScope && shape && means&& stdevs && minvals && maxvals)
	{
		...
		Input input(tensor);
		*/
		*pParameterizedTruncatedNormal = ParameterizedTruncatedNormal(*pScope, *shape, *means, *stdevs, *minvals, *maxvals, attrs);
...
void* Create_RandomGamma(std::string id, Json::Value pInputItem) {
	...
	for (int subindex = 0; subindex < iSize; ++subindex)
	{
		...
		else if (strPinName == "attrs")
		{
			...
			{
				CAttributeParser attrParser(strPinInterface, strPinInitial);
				attrs.Seed(attrParser.GetValue_bool("seed_"));
	...
	if (pScope && shape && alpha)
	{
		...
		Input input(tensor);
		*/
		*pRandomGamma = RandomGamma(*pScope, *shape, *alpha, attrs);
...
void* Create_RandomPoisson(std::string id, Json::Value pInputItem) {
	...
	if (pScope && shape && rate)
	{
		...
		Input input(tensor);
		*/
		*pRandomPoisson = RandomPoisson(*pScope, *shape, *rate, attrs);
...
void* Create_RandomUniform(std::string id, Json::Value pInputItem) {
	...
	if (pScope && shape)
	{
		...
		Input input(tensor);
		*/
		*pRandomUniform = RandomUniform(*pScope,*shape,dtype,attrs);
...
void* Create_RandomUniformInt(std::string id, Json::Value pInputItem) {
	...
	if (pScope && shape && minval && maxval)
	{
		...
		Input input(tensor);
		*/
		*pRandomUniformInt = RandomUniformInt(*pScope, *shape, *minval, *maxval, attrs);
...
void* Create_TruncatedNormal(std::string id, Json::Value pInputItem) {
	...
	if (pScope && shape )
	{
		...
		Input input(tensor);
		*/
		*pTruncatedNormal = TruncatedNormal(*pScope, *shape, dtype, attrs);
