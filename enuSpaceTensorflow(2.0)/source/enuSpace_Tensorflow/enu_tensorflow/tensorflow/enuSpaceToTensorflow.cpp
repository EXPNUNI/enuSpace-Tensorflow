...
void FindStartObject(std::map<EnuObject*, Object_Info* >* pObjectMap, CPtrList *pStartNodeList)
{
	std::map<EnuObject*, Object_Info*>::iterator vit;
	for (vit = pObjectMap->begin(); vit != pObjectMap->end(); ++vit)
	{
		Object_Info* pTar = vit->second;
		if (pTar)
		{
			if (pTar->vInputList.size() == 0)
				pStartNodeList->AddTail(pTar);
		}
	}
}

void GetNextNodeList(CPtrList* pIndexNodeList, CPtrList* pStartNodeList, CPtrList* pNextNodeList)
{
	POSITION startpos = pStartNodeList->GetHeadPosition();
	while (startpos)
	{
		Object_Info* pObject = (Object_Info*)pStartNodeList->GetAt(startpos);

		int iCount = pObject->vOutputList.size();
		for (int i = 0; i < iCount; i++)
		{
			Link_Info* pLinkInfo = pObject->vOutputList[i];

			// 기존에 등록된 객체검사.
			if (pIndexNodeList->Find(pLinkInfo->pToObject) == NULL)
			{
				// 입력정보가 완성되었는지 검사. 입력정보가 완성되지 않았다면 추가하지 않음.
				bool bInCom = false;
				int iSize = ((Object_Info*)pLinkInfo->pToObject)->vInputList.size();
				for (int i = 0; i < iSize; i++)
				{
					Link_Info* pInputLinkInfo = ((Object_Info*)pLinkInfo->pToObject)->vInputList[i];
					if (pIndexNodeList->Find(pInputLinkInfo->pFromObject) == NULL)
						bInCom = true;
				}

				if (bInCom == false)
					pNextNodeList->AddTail(pLinkInfo->pToObject);
			}
		}
		
		pStartNodeList->GetNext(startpos);
	}
}
//////////////////////////////////////////////////////////////////////////////////////////////

bool Add_OutputInfo(Object_Info* pObjectInfo, void* pOutput, int iType, std::string strname)
{
	if (pObjectInfo)
	{
		const std::map<std::string, Output_Info*>::const_iterator aLookup = pObjectInfo->pMapOutputs.find(strname);
		...
		{
			Output_Info* pOutputInfo = new Output_Info;
			pOutputInfo->type = iType;
			pOutputInfo->pOutput = pOutput;
			pObjectInfo->pMapOutputs.insert(std::pair<std::string, Output_Info*>(strname, pOutputInfo));
			return true;
		}
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////
void* Create_Scope(std::string id)
{
	int k = 0;
...
void* Create_Const(std::string id, Object_Info* pObjectInfo)
{
	Scope* pScope = m_pScope;
	Output* pOutput = new Output();
	Tensor* pTensor = nullptr;

	std::string strPinShape = "[2][2]";
	...
	GetDoubleVectorFromInitial("{{1,2},{3,4}}", arrayvals);
	...
	*pOutput = Const(*pScope, *pTensor);
	pObjectInfo->pObject = pOutput;

	Add_OutputInfo(pObjectInfo, pOutput, OUTPUT_TYPE_OUTPUT, "output");
}

void* Create_ClientSession(std::string id, Object_Info* pObjectInfo)
{
	...
	pObjectInfo->pObject = pSession;

	int iSize = pObjectInfo->vInputList.size();
	for (int i = 0; i < iSize; i++)
	{
...
bool Init_Tensorflow()
{
	...
	CString strSymbolName = L"";
	for (int index = 0; index < g_enuObject->GetSize(); ++index)
	{
		EnuObject *pobj = (EnuObject *)(g_enuObject->GetAt(index));
		strSymbolName = pobj->type;			// 구조체의 이름.
		std::string symbol = CT2CA(strSymbolName);
		int iType = GetSymbolType(symbol);

		if (iType == TF_LINK)
		{
			Link_Info* pCreateLink = new Link_Info;
			pCreateLink->pEnuLink = (tfLink*)pobj;
			m_Link_MapList.insert(std::pair<EnuObject*, Link_Info*>(pobj, pCreateLink));
		}
		else
		{
			Object_Info* pCreateObj = new Object_Info;
			pCreateObj->pEnuObject = pobj;
			pCreateObj->type = iType;
			m_Object_MapList.insert(std::pair<EnuObject*, Object_Info*>(pobj, pCreateObj));
		}
	}

	// link 루프를 통하여 객체의 입출력 리스트를 생성.
	std::map<EnuObject*, Link_Info*>::iterator vit;
	for (vit = m_Link_MapList.begin(); vit != m_Link_MapList.end(); ++vit)
	{
		Link_Info* pTar = vit->second;
		if (pTar)
		{
			Object_Info* pObj = nullptr;
			const std::map<EnuObject*, Object_Info*>::const_iterator aLookupFrom = m_Object_MapList.find(pTar->pEnuLink->from);
			const bool bExists = aLookupFrom != m_Object_MapList.end();
			if (bExists)
			{
				pTar->pFromObject = aLookupFrom->second;
				aLookupFrom->second->vOutputList.push_back(pTar);
			}
			else
			{
				std::string msg = string_format("error : link error could not find object %s", pTar->pEnuLink->from->id);
				PrintMessage(msg);
			}

			pObj = nullptr;
			const std::map<EnuObject*, Object_Info*>::const_iterator aLookupTo = m_Object_MapList.find(pTar->pEnuLink->to);
			const bool bExistsTo = aLookupTo != m_Object_MapList.end();
			if (bExistsTo)
			{
				pTar->pToObject = aLookupTo->second;
				aLookupTo->second->vInputList.push_back(pTar);
			}
			else
			{
				std::string msg = string_format("error : link error could not find object %s", pTar->pEnuLink->to->id);
				PrintMessage(msg);
			}
		}
	}
	...
	// 마지막 객체의 리스트 정보를 가져옴.
	CPtrList StartNodeList;
	FindStartObject(&m_Object_MapList, &StartNodeList);

	// 마지막 객체의 리스트
	CPtrList IndexedList;			// 정렬 리스트.
	if (StartNodeList.GetCount() > 0)
	{
		POSITION posStart = StartNodeList.GetHeadPosition();
		while (posStart)
		{
			Object_Info* pObjectInfo = (Object_Info*)StartNodeList.GetAt(posStart);
			IndexedList.AddTail(pObjectInfo);								
			StartNodeList.GetNext(posStart);
		}

		while (StartNodeList.GetCount() > 0)
		{
			CPtrList NextNodeList;
			GetNextNodeList(&IndexedList, &StartNodeList, &NextNodeList);
			StartNodeList.RemoveAll();

			POSITION posNext = NextNodeList.GetHeadPosition();
			while (posNext)
			{
				Object_Info* pNode = (Object_Info*)NextNodeList.GetAt(posNext);
				StartNodeList.AddTail(pNode);

				POSITION findpos = IndexedList.Find(pNode);
				if (findpos == NULL)
					IndexedList.AddTail(pNode);

				NextNodeList.GetNext(posNext);
			}
		}
	}

	POSITION posStart = IndexedList.GetHeadPosition();
	while (posStart)
	{
		Object_Info* pObjectInfo = (Object_Info*)IndexedList.GetAt(posStart);
...
{
	int iSymbol = pObjectInfo->type;
	std::string id = CStringToString(pObjectInfo->pEnuObject->id);

	void* pCreate = nullptr;
	switch (iSymbol)
	{
		case SYMBOL_CONST:			{		pCreate = Create_Const(id, &pObjectInfo);	break;	}
		case SYMBOL_CLIENTSESSION:	{		pCreate = Create_ClientSession(id, &pObjectInfo);	break;	}
	}
	return pCreate;
}
	...
	if (pTar->output.fetch_object.size() > 0)
	{
		...
		else
		{
			...
			for (std::vector<tensorflow::Tensor>::iterator it = outputs.begin(); it != outputs.end(); it++)
			{
				...
				strVariable = pObjet->id + ".result_" + strpinname;
				...
				{ 
					...
					case DT_DOUBLE:
					{
						...
						if (i<idis && bConnect) PrintMessage(strings::Printf("%s[%d] = %8.6f", strVariable.c_str(), i, flat(i)));
					...
					case DT_QUINT8:
					{
						...
						if (i<idis && bConnect) PrintMessage(strings::Printf("%s[%d] = %d", strVariable.c_str(), i, flat(i)));
						*((int*)pData + i) = flat(i);
						break;
					}
					case DT_UINT16:
					...
					case DT_COMPLEX64:
					{
						...
						if (i<idis && bConnect) PrintMessage(strings::Printf("%s[%d] = %8.6f+%8.6fj", strVariable.c_str(), i, flat(i).real(), flat(i).imag()));
					...
					case DT_STRING:
					{
						...
						else
						{
							...
							if (i < idis && bConnect) PrintMessage(strings::Printf("%s[%d] = %s", strVariable.c_str(), i, strTmp.c_str()));
				...
				if (bConnect)
				{
					std::string strvalue = strVariable + "=>" + it->DebugString();
				...
				if (pData)
				{
					...
					if (iType == DT_COMPLEX64 || iType == DT_COMPLEX128)
						strVariable = strVariable + strdim + "[2]";
					else
						strVariable = strVariable + strdim;
...
void ClientRunOutputList(ClientSession* pClientSession, FetchInfo* pTar, Fetch_OutputList* pOutput)
{
	...
	if (pOutput->fetch_object.size() > 0)
	{
		...
		for (std::vector<tensorflow::OutputList>::iterator it_list = pOutput->fetch_outputs.begin(); it_list != pOutput->fetch_outputs.end(); it_list++)
		{
			...
			{
				// OutputList의 출력핀 변수 이름.
				std::string strVariable;
				strVariable = pObjet->id + ".result_" + strpinname;

				// OutputList의 출력핀이 ClientSession에 연결된 경우에만, 출력 메세지를 제공.
				OutputInfo* pOutputObj = LookupFromOutputMap(pObjet, strpinname);
				bool bConnect = false;
				if (pOutputObj)
					bConnect = pOutputObj->bConnect;

				int iIndex = -1;			// OutputList의 index 정보.

				// OutputList 하나에 대하여 결과값 업데이트 수행.
				for (std::vector<tensorflow::Tensor>::iterator it = outputs.begin(); it != outputs.end(); it++)
				{
					iIndex++;				// OutputList의 index 업데이트.
					...
					for (int i = 0; i < iNum; i++)
					{
						...
						{
							auto flat = it->flat<double>();
							if (i<idis && bConnect) PrintMessage(strings::Printf("OutputList[%d] %s[%d] = %8.6f", iIndex, strVariable.c_str(), i, flat(i)));
						...
						{
							auto flat = it->flat<uint8>();
							if (i<idis && bConnect) PrintMessage(strings::Printf("OutputList[%d] %s[%d] = %d", iIndex, strVariable.c_str(), i, flat(i)));
							*((int*)pData + i) = flat(i);
							break;
						}
						case DT_UINT16:
						...
						{
							auto flat = it->flat<complex64>();
							if (i<idis && bConnect) PrintMessage(strings::Printf("OutputList[%d] %s[%d] = %8.6f+%8.6fj", iIndex, strVariable.c_str(), i, flat(i).real(), flat(i).imag()));
							*((float*)pData + i * 2) = flat(i).real();
							*((float*)pData + i * 2 + 1) = flat(i).imag();
							break;
						}
						...
						case DT_STRING:
						{
							...
							else
							{
								...
								if (i < idis && bConnect) PrintMessage(strings::Printf("OutputList[%d] %s[%d] = %s", iIndex, strVariable.c_str(), i, strTmp.c_str()));
								((std::string*)pData + i)->assign(strTmp.c_str());
							}
							break;
						}
					...
					// ClientSession에 연결된 경우에만 출력 메세지 제공.
					if (bConnect)
					{
						std::string strvalue = strVariable + "=>" + it->DebugString();
						strvalue = strings::Printf("OutputList[%d] %s", iIndex, strvalue.c_str());
						PrintMessage(strvalue);
					}

					// 인터페이스용 메모리가 생성되었다면, enuSpace에서 전달시 제공되는 Array 정보 생성 및 전달 수행.
					if (pData)
					{
						std::string strdim;
						TensorShape shape = it->shape();
						...
						if (iType == DT_COMPLEX64 || iType == DT_COMPLEX128)
							strVar = strVariable + strdim + "[2]";
						else
							strVar = strVariable + strdim;

						// 메모리 값 하나만 전달.(멀티용은 enuSpace에 값을 전달하지 않음)
						if (iIndex == 0)
							SetReShapeArrayValue(strVar, pData, iDataType, iArraySize);
...
void AddSymbolList()
{
	m_SymbolList.insert(std::pair<std::string, int>("tfLink", TF_LINK));
...
std::string GetCategoryName(int iSymbol)
{
	switch (iSymbol)
	{
	case SYMBOL_BATCHTOSPACE:
	case SYMBOL_BATCHTOSPACEND:
	case SYMBOL_BITCAST:
	case SYMBOL_BROADCASTDYNAMICSHAPE:
	case SYMBOL_CHECKNUMERICS:
	case SYMBOL_CONCAT:
	case SYMBOL_DEBUGGRADIENTIDENTITY:
	case SYMBOL_DEPTHTOSPACE:
	case SYMBOL_DEQUANTIZE:
	case SYMBOL_DIAG:
	case SYMBOL_DIAGPART:
	case SYMBOL_EDITDISTANCE:
	case SYMBOL_EXPANDDIMS:
	case SYMBOL_EXTRACTIMAGEPATCHES:
	case SYMBOL_FAKEQUANTWITHMINMAXARGS:
	case SYMBOL_FAKEQUANTWITHMINMAXARGSGRADIENT:
	case SYMBOL_FAKEQUANTWITHMINMAXVARS:
	case SYMBOL_FAKEQUANTWITHMINMAXVARSGRADIENT:
	case SYMBOL_FAKEQUANTWITHMINMAXVARSPERCHANNEL:
	case SYMBOL_FAKEQUANTWITHMINMAXVARSPERCHANNELGRADIENT:
	case SYMBOL_FILL:
	case SYMBOL_GATHER:
	case SYMBOL_GATHERV2:
	case SYMBOL_GATHERND:
	case SYMBOL_IDENTITY:
	case SYMBOL_IMMUTABLECONST:
	case SYMBOL_INVERTPERMUTATION:
	case SYMBOL_MATRIXBANDPART:
	case SYMBOL_MATRIXDIAG:
	case SYMBOL_MATRIXDIAGPART:
	case SYMBOL_MATRIXSETDIAG:
	case SYMBOL_MIRRORPAD:
	case SYMBOL_ONEHOT:
	case SYMBOL_ONESLIKE:
	case SYMBOL_PAD:
	case SYMBOL_PADV2:
	case SYMBOL_PARALLELCONCAT:
	case SYMBOL_PLACEHOLDER:
	case SYMBOL_PLACEHOLDERWITHDEFAULT:
	case SYMBOL_PREVENTGRADIENT:
	case SYMBOL_QUANTIZEANDDEQUANTIZEV2:
	case SYMBOL_QUANTIZEV2:
	case SYMBOL_QUANTIZEDCONCAT:
	case SYMBOL_QUANTIZEDINSTANCENORM:
	case SYMBOL_QUANTIZEDRESHAPE:
	case SYMBOL_RANK:
	case SYMBOL_RESHAPE:
	case SYMBOL_RESOURCESTRIDEDSLICEASSIGN:
	case SYMBOL_REVERSE:
	...
	case SYMBOL_SIZE:
	case SYMBOL_SLICE:
	case SYMBOL_SPACETOBATCH:
	case SYMBOL_SPACETOBATCHND:
	case SYMBOL_SPACETODEPTH:
	case SYMBOL_SPLIT:
	case SYMBOL_SPLITV:
	case SYMBOL_SQUEEZE:
	case SYMBOL_STACK:
	case SYMBOL_STOPGRADIENT:
	case SYMBOL_STRIDEDSLICE:
	case SYMBOL_STRIDEDSLICEASSIGN:
	case SYMBOL_STRIDEDSLICEGRAD:
	case SYMBOL_TILE:
	case SYMBOL_TRANSPOSE:
	case SYMBOL_UNIQUE:
	case SYMBOL_UNIQUEWITHCOUNTS:
	case SYMBOL_UNSTACK:
	case SYMBOL_WHERE:
	case SYMBOL_ZEROSLIKE:
		return "arrayops";
	case SYMBOL_ALLCANDIDATESAMPLER:
	case SYMBOL_COMPUTEACCIDENTALHITS:
	case SYMBOL_FIXEDUNIGRAMCANDIDATESAMPLER:
	case SYMBOL_LEARNEDUNIGRAMCANDIDATESAMPLER:
	case SYMBOL_LOGUNIFORMCANDIDATESAMPLER:
	case SYMBOL_UNIFORMCANDIDATESAMPLER:
		return "candidate-sampling-ops";
	case SYMBOL_ABORT:
	case SYMBOL_CONTROLTRIGGER:
	case SYMBOL_LOOPCOND:
	case SYMBOL_MERGE:
	case SYMBOL_NEXTITERATION:
	case SYMBOL_REFNEXTITERATION:
	case SYMBOL_REFSELECT:
	case SYMBOL_REFSWITCH:
	case SYMBOL_SWITCH:
		return "control-flow-ops";
	case SYMBOL_CLIENTSESSION:
	case SYMBOL_INPUT:
	case SYMBOL_INPUT_INITIALIZER:
	case SYMBOL_INPUTLIST:
	...
	case SYMBOL_ACCUMULATORSETGLOBALSTEP:
	case SYMBOL_ACCUMULATORTAKEGRADIENT:
	case SYMBOL_BARRIER:
	case SYMBOL_BARRIERCLOSE:
	case SYMBOL_BARRIERINCOMPLETESIZE:
	case SYMBOL_BARRIERINSERTMANY:
	case SYMBOL_BARRIERREADYSIZE:
	case SYMBOL_BARRIERTAKEMANY:
	case SYMBOL_CONDITIONALACCUMULATOR:
	case SYMBOL_DELETESESSIONTENSOR:
	case SYMBOL_DYNAMICPARTITION:
	case SYMBOL_DYNAMICSTITCH:
	case SYMBOL_FIFOQUEUE:
	...
	case SYMBOL_QUEUEDEQUEUE:
	case SYMBOL_QUEUEDEQUEUEMANY:
	case SYMBOL_QUEUEDEQUEUEUPTO:
	case SYMBOL_QUEUEENQUEUE:
	case SYMBOL_QUEUEENQUEUEMANY:
	case SYMBOL_QUEUESIZE:
	case SYMBOL_RANDOMSHUFFLEQUEUE:
	case SYMBOL_RECORDINPUT:
	case SYMBOL_SPARSEACCUMULATORAPPLYGRADIENT:
	...
	case SYMBOL_TENSORARRAYGATHER:
	case SYMBOL_TENSORARRAYGRAD:
	case SYMBOL_TENSORARRAYREAD:
	case SYMBOL_TENSORARRAYSCATTER:
	case SYMBOL_TENSORARRAYSIZE:
	case SYMBOL_TENSORARRAYSPLIT:
	case SYMBOL_TENSORARRAYWRITE:
	case SYMBOL_UNSTAGE:
		return "data-flow-ops";
	case SYMBOL_ADJUSTCONTRAST:
	case SYMBOL_ADJUSTHUE:
	case SYMBOL_ADJUSTSATURATION:
	case SYMBOL_CROPANDRESIZE:
	case SYMBOL_CROPANDRESIZEGRADBOXES:
	case SYMBOL_CROPANDRESIZEGRADIMAGE:
	case SYMBOL_DECODEBMP:
	case SYMBOL_DECODEGIF:
	case SYMBOL_DECODEJPEG:
	case SYMBOL_DECODEPNG:
	case SYMBOL_DRAWBOUNDINGBOXES:
	case SYMBOL_ENCODEJPEG:
	case SYMBOL_ENCODEPNG:
	case SYMBOL_EXTRACTGLIMPSE:
	case SYMBOL_HSVTORGB:
	case SYMBOL_NONMAXSUPPRESSION:
	case SYMBOL_QUANTIZEDRESIZEBILINEAR:
	case SYMBOL_RGBTOHSV:
	case SYMBOL_RESIZEAREA:
	case SYMBOL_RESIZEBICUBIC:
	case SYMBOL_RESIZEBILINEAR:
	case SYMBOL_RESIZENEARESTNEIGHBOR:
	case SYMBOL_SAMPLEDISTORTEDBOUNDINGBOX:
	case SYMBOL_SAMPLEDISTORTEDBOUNDINGBOXV2:
		return "image-ops";
	...
	case SYMBOL_READERNUMRECORDSPRODUCED:
	case SYMBOL_READERNUMWORKUNITSCOMPLETED:
	case SYMBOL_READERREAD:
	case SYMBOL_READERREADUPTO:
	case SYMBOL_READERRESET:
	case SYMBOL_READERRESTORESTATE:
	case SYMBOL_READERSERIALIZESTATE:
	case SYMBOL_RESTORE:
	case SYMBOL_RESTORESLICE:
	case SYMBOL_RESTOREV2:
	case SYMBOL_SAVE:
	case SYMBOL_SAVESLICES:
	case SYMBOL_SAVEV2:
	case SYMBOL_SHARDEDFILENAME:
	case SYMBOL_SHARDEDFILESPEC:
	case SYMBOL_TFRECORDREADER:
	case SYMBOL_TEXTLINEREADER:
	case SYMBOL_WHOLEFILEREADER:
	case SYMBOL_WRITEFILE:
		return "io-ops";
	case SYMBOL_ASSERT:
	case SYMBOL_HISTOGRAMSUMMARY:
	case SYMBOL_MERGESUMMARY:
	case SYMBOL_PRINT:
	case SYMBOL_SCALARSUMMARY:
	case SYMBOL_TENSORSUMMARY:
		return "logging-ops";
	case SYMBOL_ABS:
	case SYMBOL_ACOS:
	case SYMBOL_ADD:
	...
	case SYMBOL_ALL:
	case SYMBOL_ANY:
	case SYMBOL_APPROXIMATEEQUAL:
	case SYMBOL_ARGMAX:
	case SYMBOL_ARGMIN:
	case SYMBOL_ASIN:
	case SYMBOL_ATAN:
	case SYMBOL_ATAN2:
	case SYMBOL_BATCHMATMUL:
	case SYMBOL_BETAINC:
	case SYMBOL_BINCOUNT:
	case SYMBOL_BUCKETIZE:
	case SYMBOL_CAST:
	case SYMBOL_CEIL:
	case SYMBOL_COMPLEX:
	case SYMBOL_COMPLEXABS:
	case SYMBOL_CONJ:
	case SYMBOL_COS:
	case SYMBOL_COSH:
	case SYMBOL_CROSS:
	...
	case SYMBOL_EXPM1:
	case SYMBOL_FLOOR:
	case SYMBOL_FLOORDIV:
	case SYMBOL_FLOORMOD:
	case SYMBOL_GREATER:
	case SYMBOL_GREATEREQUAL:
	case SYMBOL_IGAMMA:
	case SYMBOL_IGAMMAC:
	case SYMBOL_IMAG:
	case SYMBOL_ISFINITE:
	case SYMBOL_ISINF:
	case SYMBOL_ISNAN:
	case SYMBOL_LESS:
	case SYMBOL_LESSEQUAL:
	case SYMBOL_LGAMMA:
	case SYMBOL_LINSPACE:
	case SYMBOL_LOG:
	case SYMBOL_LOG1P:
	case SYMBOL_LOGICALAND:
	case SYMBOL_LOGICALNOT:
	...
	case SYMBOL_POLYGAMMA:
	case SYMBOL_POW:
	case SYMBOL_PROD:
	case SYMBOL_QUANTIZEDOWNANDSHRINKRANGE:
	case SYMBOL_QUANTIZEDMATMUL:
	case SYMBOL_QUANTIZEDMUL:
	case SYMBOL_RANGE:
	case SYMBOL_REAL:
	case SYMBOL_REALDIV:
	case SYMBOL_RECIPROCAL:
	case SYMBOL_REQUANTIZATIONRANGE:
	case SYMBOL_REQUANTIZE:
	case SYMBOL_RINT:
	case SYMBOL_ROUND:
	case SYMBOL_RSQRT:
	case SYMBOL_SEGMENTMAX:
	case SYMBOL_SEGMENTMEAN:
	case SYMBOL_SEGMENTMIN:
	case SYMBOL_SEGMENTPROD:
	case SYMBOL_SEGMENTSUM:
	case SYMBOL_SIGMOID:
	case SYMBOL_SIGN:
	case SYMBOL_SIN:
	case SYMBOL_SINH:
	case SYMBOL_SPARSEMATMUL:
	case SYMBOL_SPARSESEGMENTMEAN:
	case SYMBOL_SPARSESEGMENTMEANGRAD:
	case SYMBOL_SPARSESEGMENTSQRTN:
	case SYMBOL_SPARSESEGMENTSQRTNGRAD:
	case SYMBOL_SPARSESEGMENTSUM:
	case SYMBOL_SQRT:
	case SYMBOL_SQUARE:
	case SYMBOL_SQUAREDDIFFERENCE:
	case SYMBOL_SUBTRACT:
	case SYMBOL_SUM:
	case SYMBOL_TAN:
	case SYMBOL_TANH:
	case SYMBOL_TRUNCATEDIV:
	case SYMBOL_TRUNCATEMOD:
	case SYMBOL_UNSORTEDSEGMENTMAX:
	case SYMBOL_UNSORTEDSEGMENTSUM:
	case SYMBOL_WHERE3:
	case SYMBOL_ZETA:
		return "math-ops";
	case SYMBOL_AVGPOOL:
	case SYMBOL_AVGPOOL3D:
	case SYMBOL_AVGPOOL3DGRAD:
	case SYMBOL_BIASADD:
	case SYMBOL_BIASADDGRAD:
	case SYMBOL_CONV2D:
	case SYMBOL_CONV2DBACKPROPFILTER:
	case SYMBOL_CONV2DBACKPROPINPUT:
	case SYMBOL_CONV3D:
	case SYMBOL_CONV3DBACKPROPFILTERV2:
	case SYMBOL_CONV3DBACKPROPINPUTV2:
	case SYMBOL_DEPTHWISECONV2DNATIVE:
	case SYMBOL_DEPTHWISECONV2DNATIVEBACKPROPFILTER:
	case SYMBOL_DEPTHWISECONV2DNATIVEBACKPROPINPUT:
	case SYMBOL_DILATION2D:
	case SYMBOL_DILATION2DBACKPROPFILTER:
	case SYMBOL_DILATION2DBACKPROPINPUT:
	case SYMBOL_ELU:
	case SYMBOL_FRACTIONALAVGPOOL:
	case SYMBOL_FRACTIONALMAXPOOL:
	case SYMBOL_FUSEDBATCHNORM:
	case SYMBOL_FUSEDBATCHNORMGRAD:
	case SYMBOL_FUSEDPADCONV2D:
	case SYMBOL_FUSEDRESIZEANDPADCONV2D:
	case SYMBOL_INTOPK:
	case SYMBOL_L2LOSS:
	case SYMBOL_LRN:
	case SYMBOL_LOGSOFTMAX:
	case SYMBOL_MAXPOOL:
	case SYMBOL_MAXPOOL3D:
	case SYMBOL_MAXPOOL3DGRAD:
	case SYMBOL_MAXPOOL3DGRADGRAD:
	case SYMBOL_MAXPOOLGRADGRAD:
	case SYMBOL_MAXPOOLGRADGRADWITHARGMAX:
	case SYMBOL_MAXPOOLWITHARGMAX:
	case SYMBOL_QUANTIZEDAVGPOOL:
	case SYMBOL_QUANTIZEDBATCHNORMWITHGLOBALNORMALIZATION:
	case SYMBOL_QUANTIZEDBIASADD:
	case SYMBOL_QUANTIZEDCONV2D:
	case SYMBOL_QUANTIZEDMAXPOOL:
	case SYMBOL_QUANTIZEDRELU:
	case SYMBOL_QUANTIZEDRELU6:
	case SYMBOL_QUANTIZEDRELUX:
	case SYMBOL_RELU:
	case SYMBOL_RELU6:
	case SYMBOL_SOFTMAX:
	case SYMBOL_SOFTMAXCROSSENTROPYWITHLOGITS:
	case SYMBOL_SOFTPLUS:
	case SYMBOL_SOFTSIGN:
	case SYMBOL_SPARSESOFTMAXCROSSENTROPYWITHLOGITS:
	case SYMBOL_TOPK:
		return "nn-ops";
	case SYMBOL_NOOP:
		return "no-op";
	case SYMBOL_DECODECSV:
	case SYMBOL_DECODEJSONEXAMPLE:
	case SYMBOL_DECODERAW:
	case SYMBOL_PARSEEXAMPLE:
	...
	case SYMBOL_RANDOMNORMAL:
	case SYMBOL_RANDOMPOISSON:
	case SYMBOL_RANDOMSHUFFLE:
	case SYMBOL_RANDOMUNIFORM:
	case SYMBOL_RANDOMUNIFORMINT:
	case SYMBOL_TRUNCATEDNORMAL:
		return "random-ops";
	case SYMBOL_ADDMANYSPARSETOTENSORSMAP:
	case SYMBOL_ADDSPARSETOTENSORSMAP:
	case SYMBOL_DESERIALIZEMANYSPARSE:
	case SYMBOL_SERIALIZEMANYSPARSE:
	case SYMBOL_SERIALIZESPARSE:
	case SYMBOL_SPARSEADD:
	case SYMBOL_SPARSEADDGRAD:
	case SYMBOL_SPARSECONCAT:
	case SYMBOL_SPARSECROSS:
	case SYMBOL_SPARSEDENSECWISEADD:
	case SYMBOL_SPARSEDENSECWISEDIV:
	case SYMBOL_SPARSEDENSECWISEMUL:
	case SYMBOL_SPARSEREDUCESUM:
	case SYMBOL_SPARSEREDUCESUMSPARSE:
	case SYMBOL_SPARSEREORDER:
	case SYMBOL_SPARSERESHAPE:
	case SYMBOL_SPARSESOFTMAX:
	case SYMBOL_SPARSESPARSEMAXIMUM:
	case SYMBOL_SPARSESPARSEMINIMUM:
	case SYMBOL_SPARSESPLIT:
	case SYMBOL_SPARSETENSORDENSEADD:
	case SYMBOL_SPARSETENSORDENSEMATMUL:
	case SYMBOL_SPARSETODENSE:
	case SYMBOL_TAKEMANYSPARSEFROMTENSORSMAP:
	case SYMBOL_SPARSEFILLEMPTYROWS:
	case SYMBOL_SPARSEFILLEMPTYROWSGRAD:
	case SYMBOL_SPARSEREDUCEMAX:
	case SYMBOL_SPARSEREDUCEMAXSPARSE:
	case SYMBOL_SPARSESLICE:
		return "sparse-ops";
	case SYMBOL_ASSIGN:
	case SYMBOL_ASSIGNADD:
	case SYMBOL_ASSIGNSUB:
	case SYMBOL_COUNTUPTO:
	case SYMBOL_DESTROYTEMPORARYVARIABLE:
	case SYMBOL_ISVARIABLEINITIALIZED:
	case SYMBOL_SCATTERADD:
	case SYMBOL_SCATTERDIV:
	case SYMBOL_SCATTERMUL:
	case SYMBOL_SCATTERNDADD:
	case SYMBOL_SCATTERNDSUB:
	case SYMBOL_SCATTERNDUPDATE:
	case SYMBOL_SCATTERSUB:
	case SYMBOL_SCATTERUPDATE:
	case SYMBOL_TEMPORARYVARIABLE:
	case SYMBOL_VARIABLE:
		return "state-ops";
	case SYMBOL_ASSTRING:
	case SYMBOL_DECODEBASE64:
	case SYMBOL_ENCODEBASE64:
	case SYMBOL_REDUCEJOIN:
	case SYMBOL_STRINGJOIN:
	case SYMBOL_STRINGSPLIT:
	case SYMBOL_STRINGTOHASHBUCKET:
	case SYMBOL_STRINGTOHASHBUCKETFAST:
	case SYMBOL_STRINGTOHASHBUCKETSTRONG:
	case SYMBOL_SUBSTR:
		return "string-ops";
	case SYMBOL_APPLYADADELTA:
	case SYMBOL_APPLYADAGRAD:
	case SYMBOL_APPLYADAGRADDA:
	case SYMBOL_APPLYADAM:
	case SYMBOL_APPLYCENTEREDRMSPROP:
	case SYMBOL_APPLYFTRL:
	case SYMBOL_APPLYGRADIENTDESCENT:
	case SYMBOL_APPLYMOMENTUM:
	case SYMBOL_APPLYPROXIMALADAGRAD:
	case SYMBOL_APPLYPROXIMALGRADIENTDESCENT:
	case SYMBOL_APPLYRMSPROP:
	case SYMBOL_RESOURCEAPPLYADADELTA:
	case SYMBOL_RESOURCEAPPLYADAGRAD:
	case SYMBOL_RESOURCEAPPLYADAGRADDA:
	case SYMBOL_RESOURCEAPPLYADAM:
	case SYMBOL_RESOURCEAPPLYCENTEREDRMSPROP:
	case SYMBOL_RESOURCEAPPLYFTRL:
	case SYMBOL_RESOURCEAPPLYGRADIENTDESCENT:
	case SYMBOL_RESOURCEAPPLYMOMENTUM:
	case SYMBOL_RESOURCEAPPLYPROXIMALADAGRAD:
	case SYMBOL_RESOURCEAPPLYPROXIMALGRADIENTDESCENT:
	case SYMBOL_RESOURCEAPPLYRMSPROP:
	case SYMBOL_RESOURCESPARSEAPPLYADADELTA:
	case SYMBOL_RESOURCESPARSEAPPLYADAGRAD:
	case SYMBOL_RESOURCESPARSEAPPLYADAGRADDA:
	case SYMBOL_RESOURCESPARSEAPPLYCENTEREDRMSPROP:
	case SYMBOL_RESOURCESPARSEAPPLYFTRL:
	case SYMBOL_RESOURCESPARSEAPPLYMOMENTUM:
	case SYMBOL_RESOURCESPARSEAPPLYPROXIMALADAGRAD:
	case SYMBOL_RESOURCESPARSEAPPLYPROXIMALGRADIENTDESCENT:
	case SYMBOL_RESOURCESPARSEAPPLYRMSPROP:
	case SYMBOL_SPARSEAPPLYADADELTA:
	case SYMBOL_SPARSEAPPLYADAGRAD:
	case SYMBOL_SPARSEAPPLYADAGRADDA:
	case SYMBOL_SPARSEAPPLYCENTEREDRMSPROP:
	case SYMBOL_SPARSEAPPLYFTRL:
	case SYMBOL_SPARSEAPPLYMOMENTUM:
	case SYMBOL_SPARSEAPPLYPROXIMALADAGRAD:
